...
| MahmoudZohdy/APICallProxy | [lib2smda#1](https://github.com/familiary/lib2smda/issues/1) | from the WinAPI-obfuscation part of the wishlist that `WinApiObfuscator` and `nt_wrapper` came from - qualified as `lib2smda#1` because the bare numbers in the rows above belong to this repository's own tracker. Its only substantial artefact is `APICallProxy.sys`, a KMDF kernel driver carrying 75 of its own functions (6 in `FileSystem.h`, 14 in `General.h`, 12 in `Network.h`, 4 in `Process.h`, 5 in `Registry.h`, 3 in `Thread.h`, 27 in `Utility.h` and 4 in `main.cpp`), and the seven user-mode `.exe` projects beside it hold one to four functions each - below `MIN_USEFUL_FUNCTIONS`, so they are not a fallback. The driver builds as `Release\|x64` and only that: it is the one `ItemDefinitionGroup` in `APICallProxy.vcxproj` that carries the link libraries, and those are `netio.lib`, `$(DDK_LIB_PATH)ntoskrnl.lib`, `hal.lib`, `wmilib.lib` and `$(KMDF_LIB_PATH)$(KMDF_VER_PATH)\WdfLdr.lib`/`WdfDriverEntry.lib`, so it needs a WDK and a KMDF. Whether the `windows-2022` runner has either is measured rather than assumed - `.github/workflows/windows-reference-data.yml` now prints a `WDK-PROBE` line per path - and this row stands until that run says otherwise. If the probe says PRESENT, this and the BlackBone driver row above are both worth reopening. A recipe would also have to record what the tree vendors and what this table's licence column cannot: `DisableDSE/hde64.h` is Vyacheslav Patkov's Hacker Disassembler Engine, which carries a copyright line and **no grant of permission** at all, and about 25 of the 75 functions are wbenny/KSOCKET's `Ks*` socket layer renamed to `APIProxy*` - that one is itself MIT, so what is missing there is the attribution rather than the permission: upstream reproduces neither the copyright line nor the licence text, and acknowledges it only as a bare URL in the README's Reference section |
| DavidBuchanan314/monomorph | [lib2smda#1](https://github.com/familiary/lib2smda/issues/1) | Linux x86-64 ELF only, and nothing here can build an ELF: `toolchain.py` registers mingw-w64 and MSVC, and the glue baseline probes msvcrt and ucrt. It would not clear the bar even if it could - its own code is four functions, `get_bit`, `decode_buf`, `inflate_buf` and `main`, 679 bytes between them, against a floor of eight. The committed artefact carries 1397 function symbols, but 1393 of them are statically linked glibc and zlib, and with no glibc baseline to subtract them they would enter under monomorph's name and duplicate `data/libzlib`. What is distinctive about the project is the 4 MB array of MD5 collision blocks, which is data rather than code; upstream points at a collision detector for identifying it |
